...
export function KeysDialog({ initial, onClose }: { initial: Keys; onClose: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#16150f]/25 backdrop-blur-sm" onClick={() => onClose()}>
      <div className="w-[560px] rounded-xl border border-line bg-panel p-7 shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          {FIELDS.map((f) => (
            <label key={f.id} className="block">
              ...
              <input
                ...
                className="mt-1.5 w-full rounded-lg border border-line bg-panel-2 px-3 py-2 font-mono text-[13px] text-text outline-none focus:border-text"
              />
            </label>
          ))}
        </div>
        ...
        <div className="mt-6 flex justify-between">
          ...
          <button onClick={() => done(keys)} className="rounded-lg bg-text px-5 py-2 text-[15px] text-[#faf8f2]">
            Save
          </button>
        </div>
      </div>
